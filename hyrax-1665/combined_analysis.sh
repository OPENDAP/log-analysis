...
cat "$combined_log_file" | jq -r '.[] | if .collectionId!=null and (.collectionId | contains("/hyrax/CMR"))  then . else "NO_CMR_PATH_FOUND" end' | grep -v NO_CMR_PATH_FOUND > "$cmr_service_records_file"
...
cat "$combined_log_file" | jq '.[] | if .http_response_code==404 then . else "not404."  end' | grep -v not404 | jq '.' > "$all_404_response_records_file"
...
cat "$combined_log_file" | jq '.[] | if .collectionId!=null and (.collectionId | startswith("/hyrax/ngap")) and .http_response_code==404  then . else "NoCollectionIdFound."  end' | grep -v NoCollectionIdFound | jq '.' > "$all_ngap_404_response_records_file"
...
cat "$combined_log_file" | jq '.[] | if .collectionId!=null and (.collectionId | startswith("/hyrax/ngap") | not) and (.collectionId | startswith("/hyrax/CMR") | not) and .http_response_code==404  then . else "NoCollectionIdFound."  end' | grep -v NoCollectionIdFound | jq '.' > "$other_404_responses_file"
...
cat "$combined_log_file" | jq '.[] | if .bes==null and .collectionId!=null and (.collectionId | startswith("/hyrax/ngap")) and .http_response_code==404 then . else "NoCollectionIdFound."  end' | grep -v NoCollectionIdFound | jq '.' > "$unmatched_ngap_404_response_records_file"
...
cat "$combined_log_file" | jq '.[] | if .collectionId!=null and (.collectionId | contains("-NSIDC_CPRD")) and .http_response_code==404 then . else "NoCollectionIdFound."  end' | grep -v NoCollectionIdFound | jq '.' > "$nsidc_cprd_404_response_records_file"
...
cat "$combined_log_file" | jq '.[] | if .collectionId!=null and (.collectionId | contains("NSIDC_CPRD")) then . else "skippy"  end' | grep -v skippy | jq '.' > "$nsidc_cprd_records_file"
...
cat "$combined_log_file" | jq '.[] | if .collectionId!=null and (.collectionId | contains("C2938661904-NSIDC_CPRD")) then . else "skippy"  end' | grep -v skippy | jq '.' > "$C2938661904_nsidc_cprd_records_file"
